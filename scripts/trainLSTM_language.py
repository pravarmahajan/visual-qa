...
def main():
    ...
    questions_train = open(
        '../data/preprocessed/questions_train2014.txt', 'r').read().decode('utf8').splitlines()
    ...
    questions_lengths_train, questions_train, answers_train = (list(t) for t in zip(
        *sorted(zip(questions_lengths_train, questions_train, answers_train))))
    ...
    model_file_name = '../models/lstm_language_only_num_hidden_units_' + \
        str(args.num_hidden_units) + '_num_lstm_layers_' + \
        str(args.num_lstm_layers) + '_dropout_' + str(args.dropout)
    ...
    for k in xrange(numEpochs):
        ...
        for qu_batch, an_batch, im_batch in zip(grouper(questions_train, batchSize, fillvalue=questions_train[0]),
                                                grouper(
                                                    answers_train, batchSize, fillvalue=answers_train[0]),
                                                grouper(images_train, batchSize, fillvalue=images_train[0])):
            # questions sorted in descending order of length
